    ...
    def get_translation(self, frame1, frame2):
        ...
        if M is not None:
            ...
            if len(dxs) > 10:
                ...
                # Sanity check: ignore jumps larger than 25% of frame
                if abs(dx) > frame1.shape[1] * 0.25 or abs(dy) > frame1.shape[0] * 0.25:
                    return 0.0, 0.0
...
class PanoramaManager:
    ...
    def _accumulate(self, frame):
        ...
        if self.sum_buffer is None:
            # Huge buffer to prevent any out-of-bounds issues
            buf_w = w * 6
            buf_h = h * 3
            self.sum_buffer = np.zeros((buf_h, buf_w, 3), dtype=np.float32)
            self.weight_buffer = np.zeros((buf_h, buf_w), dtype=np.float32)
            
            # Start exactly in the center to allow drift in any direction
            self.base_x = (buf_w - w) // 2
            self.base_y = (buf_h - h) // 2
        ...
        if curr_x < 0 or curr_y < 0 or curr_x + w > self.sum_buffer.shape[1] or curr_y + h > self.sum_buffer.shape[0]:
            event_logger.log(f"Panorama Warning: Out of bounds (x={curr_x}, y={curr_y}, canvas={self.sum_buffer.shape[1]}x{self.sum_buffer.shape[0]})")
            return
    ...
    def _finalize(self):
        ...
        event_logger.log(f"Panorama complete: Saved {filename}")
